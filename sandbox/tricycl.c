...
int32_t tricycl_solve_sp(size_t system_size, size_t num_systems,
	...
	size_t interface_iterations = tricycl_iterations(sub_size);
	...
	for(size_t s=0; s<num_systems; ++s) {
		...
		for(size_t r=0; r<sub_systems; ++r) {
			...
			for(size_t i=sub_size-3; i != 0; --i) {
				...
				// ia is static
				ib[lroff] = ratio*sub[roff+i] + diag[roff+i];
				ic[lroff] = ratio*ic[lroff];
				id[lroff] = ratio*id[lroff] + rhs[roff+i];
			} // for
		} // for
	} // for
	...
	ocl_set_kernel_arg("tridiag", "solve", 0, sizeof(cl_mem), &d_ia);
	ocl_set_kernel_arg("tridiag", "solve", 1, sizeof(cl_mem), &d_ib);
	ocl_set_kernel_arg("tridiag", "solve", 2, sizeof(cl_mem), &d_ic);
	ocl_set_kernel_arg("tridiag", "solve", 3, sizeof(cl_mem), &d_id);
	ocl_set_kernel_arg("tridiag", "solve", 4, sizeof(cl_mem), &d_ix);
