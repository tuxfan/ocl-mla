...
int main(int argc, char ** argv) {
	...
	ocl_host_start_timer("host_kernel");
...
#else
		ocl_enqueue_kernel_ndrange(OCL_PERFORMANCE_DEVICE, &token,
			1, &global_offset, &work_group_elements, &work_group_size, &event);
#endif
	} // if

	// add event to list
	ocl_add_event_to_wait_list(&list, &event);

	// wait for completion
	ocl_wait_for_events(&list);

	// stop host-side timer
	ocl_host_stop_timer("host_kernel");

	ocl_host_report_timer("host_kernel");

	// add a timer event for the kernel invocation
	ocl_add_timer("kernel", &event);

	// clear the wait list
	ocl_clear_event_wait_list(&list);
